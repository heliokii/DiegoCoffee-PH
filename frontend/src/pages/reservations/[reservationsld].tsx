...
const EditReservations = () => {
  ...
  const { reservationsId } = router.query

  useEffect(() => {
    dispatch(fetch({ id: reservationsId }))
  }, [reservationsId])
  ...
  const handleSubmit = async (data) => {
    await dispatch(update({ id: reservationsId, data }))
...
EditReservations.getLayout = function getLayout(page: ReactElement) {
  return (
      <LayoutAuthenticated
